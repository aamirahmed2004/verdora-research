"""
Contains the final functions used in the backend for the main project at: https://github.com/ribhavsharma/verdora/classification_models.py
The intention was to create one or two easy to use functions that could be seamlessy integrated into the API with a few lines of code as below:
...
In the end, we were unable to get the endpoints communicating due to having to install all the dependencies + models on each of our team member's laptops, which wasn't working. Could have possibly been fixed with docker. At the end of this file, I added manual calls for the functions to show the samples we used during the hackathon presentation.
"""
...
def get_corrected_main_annotations(main_annotations, clothing_annotations):
    ...
    for clothing_annotation in clothing_annotations:
        for main_annotation in main_annotations:
            uncertainty = None
            if main_annotation['recycling'] is not None:  
...
model.to(device)
clothing_model.to(device)

pprint(classify("sample_images/crumpled_cans.webp", model, clothing_model, recycling_models))
print(flag_ewaste("sample_images/crumpled_cans.webp", recycling_models))

pprint(classify("sample_images/random.jpg", model, clothing_model, recycling_models))
print(flag_ewaste("sample_images/random.jpg", recycling_models))

pprint(classify("sample_images/broken_phone.jpg", model, clothing_model, recycling_models))
print(flag_ewaste("sample_images/broken_phone.jpg", recycling_models))

pprint(classify("sample_images/apple_core.png", model, clothing_model, recycling_models))
print(flag_ewaste("sample_images/apple_core.png", recycling_models))

pprint(classify("sample_images/tshirt_jeans.jpg", model, clothing_model, recycling_models))
print(flag_ewaste("sample_images/tshirt_jeans.jpg", recycling_models))

pprint(classify("sample_images/empty_bottle.jpg", model, clothing_model, recycling_models))
print(flag_ewaste("sample_images/empty_bottle.jpg", recycling_models))

pprint(classify("sample_images/food_scraps.png", model, clothing_model, recycling_models))
print(flag_ewaste("sample_images/food_scraps.png", recycling_models))

pprint(classify("sample_images/glass.jpg", model, clothing_model, recycling_models))
print(flag_ewaste("sample_images/glass.jpg", recycling_models))
