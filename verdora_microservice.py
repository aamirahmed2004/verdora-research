...
def classify_for_recycling(file_name, recycling_models, visualize = True):
    """
...
"""The clothing model very confidently guesses that there are clothes in this picture. To fix this, we can calculate the IOU of the two regions (ratio of overlap to total area), and if it's high then disregard the clothing model's results."""
...
"""Initially, the bounding box formats were inconsistent (they were stored as sets, and were also in the wrong order). Now that we fixed this, we can combat the issue of VERY wrong outputs."""
...
def flag_ewaste(file_name, models, visualize=True):
    """
...
def classify_and_update_annotations(file_name, annotations, recycling_models, visualize_crops = False, visualize=True):
    """
...
import base64
def encode_image_to_base64(file_name):
    """
